...
export default class VuroxDropdown extends Component {
  componentDidMount() {
    $('.vurox-dropdown-block button, .vurox-dropdown-block .vurox-dropdown').on(
      'click',
      (e) => {
  ...
  render() {
    return (
      <div
        className={
          this.props.position + ' vurox-dropdown-block ' + this.props.className
        }
        tabIndex="3"
      >
        {this.props.children}
...
export const DropdownItemSeperator = (props) => {
  return <li className="vurox-dropdown-seperator"></li>;
};
export const DropdownBigItems = (props) => {
  return (
    <div className="dropdown-big-items d-flex justify-content-start">
      {props.children}
    </div>
  );
};
