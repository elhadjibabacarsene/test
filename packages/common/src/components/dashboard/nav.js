...
export const Navitem = (props) => {
  ...
  const submenuIcon =
    props.className === 'has-submenu' ? (
      <i className="ti-angle-right"></i>
    ) : null;
  const badge =
    props.badgde !== null ? (
      <span className={props.badge + ' ml-2'}>{props.badgeText}</span>
    ) : null;
  return (
...
export class SubNavItems extends React.Component {
  render() {
    return <ul className="vurox-sub-menu">{this.props.children}</ul>;
  }
}
export const VerticalNav = (props) => <ul>{props.children}</ul>;
