    ...
    return (
      <div
        className={
          ...
          ' vurox-admin-components-block vurox-dark vurox-chart-box-standard-padding'
        }
      >
        {this.props.children}
      </div>
  ...
  render() {
    return (
      <div className={this.props.type + ' vurox-charts-legend'}>
        <span
          style={{ backgroundColor: this.props.fill }}
...
export const processDualChartsData = (
  data1,
  data2,
  newProp1,
  newProp2,
  ...
  let chartData = data1.map((item, i) =>
    Object.assign(
      {},
      { [newProp1]: item.date, [newProp2]: item.value },
      { [newProp3]: data2[i].value }
    )
  );
  return chartData.slice(0, limit);
};
export const VuroxCustomTick = ({
  payload,
  x,
  y,
  textAnchor,
