...
function FullCalendar(props) {
  ...
  return <div className="vurox-daygrid-calendar">{showCalendar(props)}</div>;
}
...
const EventContent = ({ event }) => {
  ...
  return (
    <EventDetail>
      <div
        className="event-with-details"
        style={{
          backgroundColor: colorwop,
          borderBottom: '2px solid ' + event.extendedProps.data.color,
        }}
      >
        <div className="single-event-heading">
          <img
            src={
              event.extendedProps.data.imgUrl + event.extendedProps.data.taskImg
            }
          />
          <p>{event.title}</p>
        </div>
        <div className="single-event-content">
          ...
          <img
            src={
              event.extendedProps.data.imgUrl +
              event.extendedProps.data.clientImg
            }
          />
          ...
          <span
            style={{
              backgroundColor: colorwop,
              color: event.extendedProps.data.color,
            }}
          >
            {event.extendedProps.data.orderStatus}
          </span>
        </div>
      </div>
    </EventDetail>
...
const vuroxRemakeCalendar = (info) => {
  ...
  for (i = 0; i <= duration._data.days; i++) {
    ...
    $(row)
      .find('.fc-day-top[data-date="' + c.format('YYYY-MM-DD') + '"]')
      .addClass('less-left')
      .append(
        '<a href="#" className="fc-event-dot" onclick="return false;" ' +
          'style="background-color: ' +
          info.event.extendedProps.data.color +
          ';" ' +
          'title="' +
          title +
          '"></a>'
